...
Integrate_Data <- function(mutile_Group, RT_Tolerance_1 = 10, mz_Tolerance_1 = 0.1, RT_Tolerance_2 = 10, mz_Tolerance_2 = 0.1){
    ...
    if(length(all_condition)!=0){
      ...
      for(p in 1:length(undata_all)){
        ...
        if (undata_all[[p]]!=0){
          ...
          condition_s2_q <- which(abs(as.numeric(undata_s2_q[,"mz"])-as.numeric(undata_all[[p]][, "mz"])) < (0.5*mz_Tolerance_2) & abs(as.numeric(undata_s2_q[,"rt"])-as.numeric(undata_all[[p]][, "rt"])) < RT_Tolerance_2, arr.ind=TRUE)
  ...
  row.names(mutile_align) <- mutile_align[, 1]
  mutile_align_all <- mutile_align[, -1]
