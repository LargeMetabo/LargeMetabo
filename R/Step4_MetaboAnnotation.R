...
#' Title Metabo_Annotation
#'
#' @param AnnotaData The value of mass-to-charge ratio for MS1 spectra for metabolite annotation.
#' @param masstole The tolerance of m/z for metabolite annotation, and the unit is ppm or Da.
#' @param toleUnit The unit of m/z for metabolite annotation. The unit is 1 for Da and 2 for ppm.
#' @param annotaDB The database for metabolite annotation. The database can be metlin or hmdb.
#' @param ionMode The mode of m/z for metabolite annotation. The mode can be pos, neg or neu.
#'
#' @return The result of metabolite annotation for the adducts and charges for MS1 spectra.
#' @export
#'
#' @examples AnnotaMS <- AnnotaData$AnnotaMS
#' MetaboAResult <- Metabo_Annotation(AnnotaMS)
#' MetaboAResult$`M+H-2H2O`
#'
#'
Metabo_Annotation <- function(AnnotaData, masstole = 0.05, toleUnit = 1, annotaDB = "metlin", ionMode  = "pos"){
...
#' Title Annota_Tandem
#'
#' @param ParentMass The m/z value of parent ion for metabolite annotation of MS/MS.
#' @param TandemData The MS/MS peak list (including the matrix of m/z values and intensities of MS/MS) for metabolite annotation of MS/MS.
#' @param massTandem The tolerance of parent ion for metabolite annotation of MS/MS, and the unit is ppm or Da.
#' @param toleUnitTandem The unit of parent ion for metabolite annotation of MS/MS. The unit is 1 for Da and 2 for ppm.
#' @param massmzTandem The tolerance of MS/MS peak for metabolite annotation of MS/MS, and the unit is ppm or Da.
#' @param toleUnitmzTandem The unit of MS/MS peak for metabolite annotation of MS/MS. The unit is 1 for Da and 2 for ppm.
#' @param ModeTandem The ionization mode for metabolite annotation of tandem mass spectrum. The mode can be Positive or Negative.
#' @param ionEnergy The CID energy for metabolite annotation of MS/MS. The mode can be low(10V), Medium(25V), High(40V), or All.
#'
#' @return The parameters for metabolite annotation of MS/MS.
#' @export
#'
#' @examples ParentMass <- AnnotaData$ParentMass
#' TandemData <- AnnotaData$TandemData
#' AnnotaParamTandem <- Annota_Tandem(ParentMass, TandemData)

Annota_Tandem <- function(ParentMass = 181.04, TandemData, massTandem = 0.1, toleUnitTandem = 1, massmzTandem = 0.5, toleUnitmzTandem = 1, ModeTandem = "Positive", ionEnergy = "low(10V)"){
  ...
  # ParentMass,label="Parent Ion Mass (Da)",value=181.04,step=0.0001,min=0

  paramTandem$ParentMass <- ParentMass

  # massTandem,label="Tolerance of Parent Ion",value=0.1,step=0.0001,min=0

  paramTandem$massTandem <- massTandem

  # toleUnitTandem,label="Unit",choices=list("Da"=1,"ppm"=2),selected=1,inline=T

  if (toleUnitTandem==1){
    paramTandem$toleUnitTandem <- "Da"
  }else if (toleUnitTandem==2){
    paramTandem$toleUnitTandem <- "ppm"
  }

  # massmzTandem",label="Tolerance of Mass/Charge",value=0.5,step=0.0001,min=0

  paramTandem$massmzTandem <- massmzTandem

  # "toleUnitmzTandem",label="Unit",choices=list("Da"=1,"ppm"=2),selected=1,inline=T

  if (toleUnitmzTandem==1){
    paramTandem$toleUnitmzTandem <- "Da"
  }else if (toleUnitmzTandem==2){
    paramTandem$toleUnitmzTandem <- "ppm"
  }

  #
  #param$annotaDB <- input$annotaDB
  #param$ionMode  <- input$ionMode
  # ModeTandem, label="Ionization Mode", choices=c("Positive"="Positive", "Negative"="Negative")))

  if (ModeTandem=="Positive"){
    paramTandem$ModeTandem <- "Positive"
  }else if(ModeTandem=="Negative"){
    paramTandem$ModeTandem <- "Negative"
  }else{
    paramTandem$ModeTandem <- NULL
  }
  ...
  if (ionEnergy=="low(10V)"){
    paramTandem$ionEnergy <- "low(10V)"
  }else if(ionEnergy=="Medium(25V)"){
    paramTandem$ionEnergy <- "Medium(25V)"
  }else if(ionEnergy=="High(40V)"){
    paramTandem$ionEnergy <- "High(40V)"
  }else if(ionEnergy=="All"){
    paramTandem$ionEnergy <- "All"
  }else{
    paramTandem$ionEnergy <- NULL
  }

  # Include_predicted", label=strong("Include predicted spectra"), value = FALSE, width = NULL

  paramTandem$Include_predicted <- FALSE

  # TandemData
  colnames(TandemData) <- c("V1","RI")

  mzTandemlist <- as.data.frame(TandemData)
  paramTandem$mzTandemlist <- mzTandemlist

  AnnotaParamTandem <- paramTandem

  return(AnnotaParamTandem)

}


##########
AnnotationTandem <- function(AnnotaParamTandem){

  calcMZLowerTandem <- NULL
  calcMZLowerTandem <- function(mz, unit, tole){
    if (unit=="ppm")
      return (mz/(1+tole*1e-6))
    else
      return (mz-tole)
  ...
  calcMZUpperTandem <- NULL
  calcMZUpperTandem <- function(mz, unit, tole){
    if (unit=="ppm")
      return (mz/(1-tole*1e-6))
    else
      return (mz+tole)
  ...
  mzT <- AnnotaParamTandem$ParentMass
  unitT <- AnnotaParamTandem$toleUnitTandem
  toleT <- AnnotaParamTandem$massTandem

  #
  queryResult <- NULL
  minMzT <- calcMZLowerTandem(mzT, unitT, toleT)
  maxMzT <- calcMZUpperTandem(mzT, unitT, toleT)

  #
  #load("D:/Code/Rpackage/LargeMetabo/data/annotationTandem_data.RData")
  ...
  annotationTandem_data <- rbind(annotationTandom_data01, annotationTandom_data02)


  Annotation_db <- annotationTandem_data
  options(digits=2)
  AnnotationTandem <- Annotation_db[which(Annotation_db$Mass>=minMzT & Annotation_db$Mass<=maxMzT & Annotation_db$Energy == AnnotaParamTandem$ionEnergy & Annotation_db$Ionization == AnnotaParamTandem$ModeTandem),c("LargeMetabo_ID", "Label", "Spectrum", "Mass", "Name", "Annotation")]

  return(AnnotationTandem)

}


##########
#' Title annota_Data_Tandem
#'
#' @param AnnotaParamTandem The parameters of metabolite annotation of MS/MS.
#'
#' @return The table of metabolite annotation of MS/MS.
#' @export
#'
#' @examples ParentMass <- AnnotaData$ParentMass
#' TandemData <- AnnotaData$TandemData
#' AnnotaParamTandem <- Annota_Tandem(ParentMass, TandemData)
#' annota_Data_Tandem(AnnotaParamTandem)

annota_Data_Tandem <- function(AnnotaParamTandem){

  ####
  AnnotationTables <- AnnotationTandem(AnnotaParamTandem)
  ...
  for(i in 1:dim(AnnotationTables)[1]){
    ...
    res_co <- NULL
    spectrumTandem <- AnnotationTables$Spectrum[i]

    s1 <- unlist(strsplit(spectrumTandem,";"))
    ...
    ###
    TandemData <- AnnotaParamTandem$mzTandemlist

    res_co <- cossim(TandemData, s2)
  ...
  AnnotationTandem <- rbind(NULL,NULL)
  for (i in 1:dim(AnnotationTables)[1]){

    bi_a <- cbind(AnnotationTables[i,],res_MSMS[i])

    AnnotationTandem <- rbind(AnnotationTandem, bi_a)

  }

  colnames(AnnotationTandem) <- c(colnames(AnnotationTables),"Fit")

  AnnotationTandem1 <- AnnotationTandem[,-which(colnames(AnnotationTandem)=="Spectrum")]

  Include_predicted <- AnnotaParamTandem$Include_predicted


  if (Include_predicted==FALSE){

    AnnotationTandem2 <- AnnotationTandem1[,-which(colnames(AnnotationTandem1)=="Label")]

  }else{
    AnnotationTandem2 <- AnnotationTandem1[AnnotationTandem1[,"Label"]=="experimental",-which(colnames(AnnotationTandem1)=="Label")]
  }

  annotationResultTandem <- AnnotationTandem2

  ####
  mass <- AnnotaParamTandem$mzTandemlist

  table_s <- annotationResultTandem

  annotaDataTandem <- table_s[rev(order(table_s$Fit)),]

  return(annotaDataTandem)
...
#' Title Annota_Tandem_plot
#'
#' @param AnnotaParamTandem The parameters of metabolite annotation of MS/MS.
#' @param TandemData The MS/MS peak list (including the matrix of m/z values and intensities of MS/MS) for metabolite annotation of MS/MS.
#'
#' @return The plot of results for metabolite annotation of MS/MS.
#' @export
#'
#' @examples ParentMass <- AnnotaData$ParentMass
#' TandemData <- AnnotaData$TandemData
...
Annota_Tandem_plot <- function(AnnotaParamTandem, TandemData){

  TandemData <- AnnotaData$TandomData

  mass <- AnnotaParamTandem$mzTandemlist

  annotResTan <- AnnotationTandem(AnnotaParamTandem)

  AnnotationTables <- annotResTan
  plot_cand <- AnnotationTables$LargeMetabo_ID

  shortlist <- list()
  shortlist[[1]] <- TandemData
