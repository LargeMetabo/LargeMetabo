...
OPLSDA_test <- function(mat, label, cross = 7, per = 0, scale = "standard") {
  X <- t(mat)
  Y <- as.factor(label)
  oplsda <- opls(X, Y, predI = 1, orthoI = NA, fig.pdfC = "interactive", crossvalI = cross, permI = per, scaleC = scale)
  res <- oplsda@vipVn
...
#' @param method The method for marker identification. The method can be FC, PLS-DA, OPLS-DA, t-test, CHIS, CFS, ENTROPY, LMEB, RELIEF, RF, SAM, SVMRFE or WRST.
#'
#' @return The result of marker identification for large-scale metabolomic data.
#' @export
#'
...
#' Title Marker_Assess
#'
#' @param finalData The matrix of dataset for performance assessment of marker identification.
#' @param finalLabel The label of dataset for performance assessment of marker identification.
#' @param method The method used for identifying markers for performance assessment. The method can be FC, PLS-DA, OPLS-DA, t-test, CHIS, CFS, ENTROPY, LMEB, RELIEF, RF, SAM, SVMRFE or WRST.
#'
#' @return The ROC curve and AUC value in the SVM classification model using identified markers.
#' @export
#'
#' @examples finalData <- MarkerData$finalData
#' finalLabel <- MarkerData$finalLabel
...
#'
Marker_Assess <- function(finalData, finalLabel, method = "PLS-DA") {
